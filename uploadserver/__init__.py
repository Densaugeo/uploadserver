...
class UploadServer:
    ...
    def send_upload_page(self, handler):
        handler.send_response(http.HTTPStatus.OK)
        handler.send_header('Content-Type', 'text/html; charset=utf-8')
        handler.send_header('Content-Length', len(self.UPLOAD_PAGE))
        handler.end_headers()
        handler.wfile.write(self.UPLOAD_PAGE)

    def receive_upload(self, handler):
        ...
        for field in fields:
            ...
            if self.token:
                ...
                if 'token' not in form or form['token'].value != self.token:
            ...
            if filename:
                with open(pathlib.Path(self.directory) / filename, 'wb') as f:
    ...
    def intercept_first_print(self):
        # Use the right protocol in the first print call
    ...
    def ssl_wrap(self, socket):
        context = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)
        server_root = pathlib.Path(self.directory).resolve()
        
        # Server certificate handling
        server_certificate = pathlib.Path(self.server_certificate).resolve()
        ...
        if self.client_certificate:
            ...
            client_certificate = pathlib.Path(self.client_certificate).resolve()
    ...
    def serve_forever(self):
        class SimpleHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
            def do_GET(inner_self):
                if inner_self.path == '/upload':
                    self.send_upload_page(inner_self)
                else:
                    http.server.SimpleHTTPRequestHandler.do_GET(inner_self)
            
            def do_POST(inner_self):
                if inner_self.path == '/upload':
                    result = self.receive_upload(inner_self)
                    if result[0] < http.HTTPStatus.BAD_REQUEST:
                        inner_self.send_response(result[0], result[1])
                        inner_self.end_headers()
                    else:
                        inner_self.send_error(result[0], result[1])
                else:
                    inner_self.send_error(http.HTTPStatus.NOT_FOUND, 'Can only POST to /upload')
        ...
        class CGIHTTPRequestHandler(http.server.CGIHTTPRequestHandler):
            def do_GET(inner_self):
                if inner_self.path == '/upload':
                    ...
                    http.server.CGIHTTPRequestHandler.do_GET(inner_self)
            ...
            def do_POST(inner_self):
                if inner_self.path == '/upload':
                    ...
                    http.server.CGIHTTPRequestHandler.do_POST(inner_self)
        ...
        if self.cgi:
            ...
            handler_class = functools.partial(SimpleHTTPRequestHandler, directory=self.directory)
        ...
        if sys.version_info.major == 3 and sys.version_info.minor < 8:
            ...
            class CustomHTTPServer(DefaultHTTPServer):
                def server_bind(inner_self):
                    bind = super().server_bind()
                    if self.server_certificate:
                        inner_self.socket = self.ssl_wrap(inner_self.socket)
                    return bind
    ...
    def __init__(self, port=8000, cgi=False, bind=None, token=None, server_certificate=None, client_certificate=None, directory=os.getcwd()):
        self.port = port
        self.cgi = cgi
        self.bind = bind
        self.token = token
...
def main():
    ...
    parser.add_argument('--server-certificate', '--certificate', '-c',
        help='Specify HTTPS server certificate [default: none]')
    parser.add_argument('--client-certificate',
        help='Specify HTTPS client certificate for mutual TLS [default: none]')
    ...
    UploadServer(port=args.port, cgi=args.cgi, bind=args.bind, token=args.token, server_certificate=args.server_certificate, client_certificate=args.client_certificate, directory=args.directory).serve_forever()
